...
##############################################################################
### TESTS FOR softmax_regression_epoch_cuda()

def test_softmax_regression_epoch_cuda():
    try:
        from simple_ml_cuda import cuda_available, softmax_regression_epoch_cuda
    except ImportError:
        pytest.skip("CUDA extension has not been compiled")
    ...
    softmax_regression_epoch_cuda(X, y, theta_cuda, lr=lr, batch=batch)
    np.testing.assert_allclose(theta_cuda, theta_reference, rtol=1e-4, atol=1e-4)
